...
De klant heeft Python versie 3.10 nodig, omdat de frontend-code van de klant Python nodig heeft om de toepassing te bouwen.

Nochtans, wanneer het opstellen van de pijpleiding in de Manager van de Wolk, ontbreekt de bouwstijl met een fout - `[` 1`]`
...
Als Python helemaal niet is geïnstalleerd, kunt u dit doen met Maven, na de informatie in `[` 1`]` .

Als u een Maven-project wilt maken dat de exec-plug-in gebruikt om Python3 te installeren, moet u uw pom.xml dienovereenkomstig configureren.
...
Deze Maven configuratie zal de &quot;apt-get update&quot;en &quot;apt-get installatie -y python3&quot;bevelen tijdens de validate fase van de Maven bouwstijllevenscyclus in werking stellen. Dit werkt alleen als de build wordt uitgevoerd in een omgeving waar de opdracht &quot;apt-get&quot; beschikbaar is en de gebruiker die de build uitvoert, voldoende rechten heeft om pakketten te installeren.

En als u Python 3.11 moet installeren, zou pom.xml als volgt moeten kijken:
