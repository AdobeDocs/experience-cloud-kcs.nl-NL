...
[&lt;- Vorige: Basisbestandsindeling](https://experienceleague.adobe.com/docs/experience-cloud-kcs/kbarticles/KA-17502.html)
...
| FILENAME`_renders.any` | `/etc/httpd/conf.dispatcher.d/renders/` | `*_renders.any` bestanden worden opgenomen in de `conf.dispatcher.d/enabled_farms/*_farm.any` bestanden. Zij specificeren IP, haven, en onderbrekingsmontages voor elke renderer. Een juiste renderer kan een livecycle-server of een AEM zijn waar de verzender de aanvragen van |
...
Zoals u hierboven kunt zien, omvat dit het hoogste niveau `dispatcher.any` bestand voor onze verzendermodule om de configuratiebestanden van deze module op te halen `/etc/httpd/conf.dispatcher.d/dispatcher.any`
...
![In deze afbeelding ziet u hoe een .vhost-bestand bestanden bevat van variabelen, whitelists en mappen opnieuw schrijven](assets/Apache-Webserver-AMS-Vhost-Includes.png "Apache-Webserver-AMS-Vhost-Includes")
...
![<FILENAME>_farm.any zal sub.any dossiers omvatten om een landbouwbedrijfconfiguratie te voltooien.  In dit beeld kunt u zien dat een landbouwbedrijf elk hoofdniveau sectiedossiers geheim voorgeheugen, klantheaders, filters, teruggeeft en gastheren.om het even welke dossiers zal omvatten](assets/Apache-Webserver-AMS-Farm-Includes.png "Apache-Webserver-AMS-Farm-Includes")
...
[Volgende -> Cache begrijpen](https://experienceleague.adobe.com/docs/experience-cloud-kcs/kbarticles/KA-17912.html)