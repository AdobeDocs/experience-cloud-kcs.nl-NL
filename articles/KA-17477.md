...
Dit document wordt uitgesplitst en legt alle configuratiebestanden uit die zijn geïmplementeerd in een standaard geïntegreerde Dispatcher-server die is ingericht in Adobe Managed Services. Het gebruik ervan, de naamgevingsconventie, enz.
...
| `<` FILENAME`>` _renders.any | /etc/httpd/conf.dispatcher.d/renders/ | \*_renders.any files are included within the conf.dispatcher.d/enabled_farm/\*_farm.any files. Zij specificeren IP, haven, en onderbrekingsmontages voor elke renderer. Een juiste renderer kan een levenscyclusserver of om het even welke AEM systemen zijn waar de Dispatcher de verzoeken van kan halen/volmacht |
...
Als plaatsvoorbeeld voor ExampleCo worden twee configuratiedossiers gecreeerd door de ontwikkelaars van de configuraties van de Verzender.
...
- Geef dit bestand een naam als het een IP-bestand met whitelist-regels is `<description>_whitelist.rules` als achtervoegsel en extensie. Deze naamgevingsconventie geeft een beschrijving van wat het is en of het een set IP-overeenkomende regels is.
...
U kunt bijvoorbeeld een bestand met de naam .rules, .any of .vhost plaatsen in de map autoInclude van `/etc/httpd/conf.d/` zou geen effect hebben.
...
Bij het uitbreiden van functionaliteit en configuraties in Apache Webserver geïnstalleerd op Enterprise Linux, hebt u enkele belangrijke <b>include-orders</b> je zult het willen begrijpen .

A. <u>Inclusief basislijn Apache</u>

Apache binary start met `httpd.conf` die een `includeoptional` aan de `conf.d/*.conf` en `conf.modules.d/*.conf` mappen.

![](assets/0f88d9ec-8014-ef11-9f89-6045bd06fa9d.png)

Zoals u ziet in het diagram boven de httpd-binaire code, ziet u alleen naar de `httpd.conf` bestand als configuratiebestand. Dat bestand bevat de volgende instructies:
...
Wanneer Apache wordt geladen, wordt deze in de `/etc/httpd/conf.modules.d/02-dispatcher.conf` en dat bestand bevat het binaire bestand `/etc/httpd/modules/mod_dispatcher.so` in de actieve staat.
...
Zoals u hierboven kunt zien, is dit ook het hoogste verzendingsprogramma. Elk bestand voor onze Dispatcher-module om de configuratiebestanden van de Dispatcher op te halen `/etc/httpd/conf.dispatcher.d/dispatcher.any`
