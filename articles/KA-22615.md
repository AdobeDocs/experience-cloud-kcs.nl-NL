...
# AEM GraphQl-configuratieproblemen
...
De klant is bezig met het ontwikkelen van een website die via AEM oplossing gegevens uit AEM instanties moet ophalen.

Aan opstelling het eindpunt, het leerprogramma in `[` 1`]`  is gevolgd.

Na het publiceren van alle inhoudsfragmenten, alle modellen, inhoud, verwijzingen, activa en vragen zoals die in `[` 2`]` Wanneer u een inhoudsfragment probeert op te halen uit het publicatieexemplaar, wordt een fout van 404 ontvangen, waardoor de inhoud niet van de uitgever kan worden gelezen.
...
`[` 1`]`  aem_base_version: 6.5 aem_envId: xxxxxxxxxx aem_envType: dev aem_program_id: xxxxxx aem_release_id: xxxxxxx aem_service: cm-pxxxx-exxxxx aem_huurder: ns-team-aem-cm-prd-nxxxxxx aem_tier: achtergrond end: publishfarm/- cache: <b>geblokkeerd</b>
cluster: `<` cluster`>`
code: - file_path: /var/log/aem/dispatcher.log host: publish-pxxxxxx-exxxxxxx.adobeaemcloud.com method: POST namespace: ns-team-aem-cm-prd-nxxxxxx orig_time: 25/Jul/2023:15:39:11 +0000 pad: /content/cq:graphql/graphql/endpoint.json pod_name: `<` pod_name`>`
pod_uid: `<` pod_uid`>`
