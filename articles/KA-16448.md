...
keywords: "KCS, migreer gebruikers, groepen, ACLs, tussen AEM instanties, Adobe Experience Manager, hoe te, AEM"
...
Leer hoe te om Gebruikers en Groepen met ACL toestemmingen in AEM van één server aan een andere of van één AEM instantie aan een andere te migreren.
...
6. De paden op de regel aanpassen aan de instantie (toevoegen `segment-old:` vóór het pad, indien nodig, zie [hier](https://jackrabbit.apache.org/oak/docs/migration.html)):
...
2. Ga naar de <b>Pakketbeheer</b>, `http://host:port/crx/packmgr/index.jsp`en aanmelden als *admin*.
...
13. (Alleen upgrade) Als u een migratie naar een nieuwere AEM uitvoert, installeert u een nieuwe lokale AEM <b>van de oude versie</b> (<b>with `nosamplecontent`</b>) en installeer zowel het gebruikerspakket als het groepspakket daar. Voer vervolgens een upgrade ter plekke uit naar de nieuwe versie op die instantie. Hiermee worden de gebruikers geconverteerd naar de nieuwe oak-weergave. Na de upgrade op de locatie herverpakken de gebruikers opnieuw om ze naar de gewenste geüpgrade instantie te verzenden. Doe het zelfde voor de gebruikersgroepen.
...
3. Ga naar `http://aem-host:port/crx/packmgr/index.jsp` en aanmelden als *admin*.
...
1. Ga naar `http://src-aem-host:port/crx/packmgr/index.jsp,` en aanmelden als *admin*.
2. Een pakket maken met de naam *ACL-migratie*.
3. Klik op de knop <b>Bewerken</b> knop.
4. Selecteer de <b>Geavanceerd</b> en -set <b>Wisselstroomafhandelingsmodus</b> tot *Samenvoegen*.
5. Opslaan.
